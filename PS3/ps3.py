...
def get_word_score(word, n):
    ...
    if word != "":
        ...
        for char in word:
            sum_one += SCRABBLE_LETTER_VALUES[char]
...
def update_hand(hand, word):
    ...
    for char in word:
        if char in my_hand.keys():
            if my_hand[char] > 0:
                my_hand[char] -= 1
    my_ha = my_hand.copy()
    for ch in my_ha.keys():
        if my_hand[ch] == 0:
            del(my_hand[ch])
...
def substitute_hand(hand, letter):
    ...
    if letter in my_hand.keys():
        val = my_hand[letter]
        x = letter
        while x in my_hand.keys():
            x = random.choice(ALPHABETS)

        del(my_hand[letter])
        my_hand[x] = val

    return my_hand  # TO DO... Remove this line when you implement this function
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    while(number_hands > 0):
        ...
        display_hand(hand)
        if sub_flag:
            resp = input("Would you like to substitute a letter? ").lower()
            if resp == "yes":
                let = input("Which letter would you like to replace: ")[0:1]
        ...
        print('---------------------------------')
        if rep_flag:
            resp = input("Would you like to replay the hand? ").lower()
            if resp == "yes":
                rep_flag = False
                total_score += play_hand(hand, word_list)
